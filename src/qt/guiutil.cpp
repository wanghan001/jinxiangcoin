...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("jinxiangcoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("jinxiangcoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "jinxiangcoin:");
    }
...
{
    QString ret = QString("jinxiangcoin:%1").arg(info.address);
    int paramCount = 0;
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "jinxiangcoin.desktop";
    return GetAutostartDir() / strprintf("jinxiangcoin-%s.lnk", chain);
}
